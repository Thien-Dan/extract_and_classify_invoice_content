...
from module.finetune.inferencer import LayoutLMInferencer
...
class InvoicePipeline:
    def __init__(self, device='cpu', skew_threshold=1.5):
        ...
        print("4. Loading LayoutLM Inferencer (4 Labels)...")
        self.inferencer = LayoutLMInferencer(model_dir="saved_models/layoutlm_mcocr_4labels")
    ...
    def process(self, image_path, output_image_path=None):
        ...
        final_labels = self.inferencer.predict(layoutlm_input["words"], layoutlm_input["bboxes"])
        
        results_list = []
        for w, bbox, label in zip(layoutlm_input["words"], layoutlm_input["bboxes"], final_labels):
            results_list.append({"text": w, "label": label})
